...
void IncrementalMapper::EndReconstruction(const bool discard) {
	...
	{
		std::cout << "Exception caught: IncrementalMapper::EndReconstruction" << std::endl;
...
IncrementalMapper::LocalBundleAdjustmentReport IncrementalMapper::AdjustLocalBundle(const Options& options, const BundleAdjustmentOptions& ba_options, const IncrementalTriangulator::Options& tri_options, const image_t image_id, const std::unordered_set<point3D_t>& point3D_ids, double& finalCost)
{
#ifdef USE_HIERARCHICAL_WEIGHT_BA

	CHECK_NOTNULL(reconstruction_);
	CHECK(options.Check());
	const size_t numImages = database_cache_->NumImages();
	std::unordered_map<image_t, std::vector<std::pair<image_t, size_t>>> correspondeImages;
	correspondeImages.reserve(numImages);
	auto compare = [](const std::pair<image_t, size_t>& a, const std::pair<image_t, size_t>& b) {
		return a.second > b.second;
		};
	const std::unordered_map<image_t, Image>& images = database_cache_->Images();
	for (auto first = images.begin(); first != images.end(); first++)
	{
		const image_t firstID = first->first;
		if (!reconstruction_->IsImageRegistered(firstID))
		{
			continue;
		}
		for (auto second = images.begin(); second != images.end(); second++)
		{
			if (second == first)
			{
				continue;
			...
			const image_t secondID = second->first;
			if (!reconstruction_->IsImageRegistered(secondID))
			{
				continue;
			}

			const size_t numCorr = database_cache_->CorrespondenceGraph()->NumCorrespondencesBetweenImages(firstID, secondID);
			if (numCorr != 0)
			{
				correspondeImages[firstID].push_back({ secondID, numCorr });
			}
		}
		std::sort(correspondeImages[firstID].begin(), correspondeImages[firstID].end(), compare);
	}

	std::unordered_map<image_t, double> weights;
	std::vector<std::unordered_set<image_t>> layers;
	weights.reserve(numImages);
	weights[image_id] = 1;

	std::unordered_set<image_t> totalImages;
	std::unordered_set<image_t> preLayerImages;
	preLayerImages.insert(image_id);
	totalImages.insert(image_id);

	image_t curLayerIndex = 1;
	const size_t topN = 8;
	const size_t maxNumLayers = 2;
	size_t maxNumLocalBAImages = 50;
	if (!reconstruction_->localBATimes.empty())
	{
		if (reconstruction_->localBATimes.back()[0] > 1000)
		{
			maxNumLocalBAImages = std::max((int)(reconstruction_->numLocalBAImages.back() * 0.75), 6);
		}
		else if (reconstruction_->localBATimes.back()[0] > 800)
		{
			maxNumLocalBAImages = std::max((int)(reconstruction_->numLocalBAImages.back() * 0.85), 6);
		}
		else if (reconstruction_->localBATimes.back()[0] < 500)
		{
			maxNumLocalBAImages = std::max((int)(reconstruction_->numLocalBAImages.back() * 1.5), 6);
		}
	}
	while (curLayerIndex <= maxNumLayers && !preLayerImages.empty())
	{
		std::unordered_set<image_t> thisLayerImages;
		for (image_t preLayerImageID : preLayerImages)
		{
			if (correspondeImages[preLayerImageID].size() < topN)
			{
				for (size_t i = 0; i < correspondeImages[preLayerImageID].size(); i++)
				{
					if (totalImages.find(correspondeImages[preLayerImageID][i].first) == totalImages.end())
					{
						thisLayerImages.insert(correspondeImages[preLayerImageID][i].first);
						totalImages.insert(correspondeImages[preLayerImageID][i].first);
					}
				}
			}
			else
			{
				size_t count = 0, curIndex = 0;
				while (count < topN && curIndex < correspondeImages[preLayerImageID].size())
				{
					if (totalImages.find(correspondeImages[preLayerImageID][curIndex].first) == totalImages.end())
					{
						thisLayerImages.insert(correspondeImages[preLayerImageID][curIndex].first);
						totalImages.insert(correspondeImages[preLayerImageID][curIndex].first);
						count++;
					}
					curIndex++;
				}
		...
		for (size_t thisLayerImageID : thisLayerImages)
		{
			if (curLayerIndex == maxNumLayers)
			{
				if (weights.size() < maxNumLocalBAImages)
				{
					weights[thisLayerImageID] = 10;
				}
			}
			else if (curLayerIndex == 1)
			{
				if (weights.size() < maxNumLocalBAImages)
				{
					weights[thisLayerImageID] = 1;
				}
			}
			else
			{
				if (weights.size() < maxNumLocalBAImages)
				{
					weights[thisLayerImageID] = 2 * (curLayerIndex - 1);
				}
			}
		}
		if (!thisLayerImages.empty())
		{
			layers.push_back(thisLayerImages);
		}
		preLayerImages = thisLayerImages;
		curLayerIndex++;
	}
	for (auto layersIt = layers.begin(); layersIt != layers.end();)
	{
		for (auto it = layersIt->begin(); it != layersIt->end();)
		{
			if (weights.find(*it) == weights.end())
			{
				it = layersIt->erase(it);
			}
			else
			{
				it++;
		...
		if (layersIt->empty())
		{
			layersIt = layers.erase(layersIt);
		}
		else
		{
			layersIt++;
	...
	if (weights.size() >= 2)
	{
		...
		ba_config.weights = weights;
		for (const auto& pair : weights)
		{
			ba_config.AddImage(pair.first);
		}
		...
		if (weights.size() == 2)
		{
			ba_config.SetConstantCamPose(*(layers[0].begin()));
			ba_config.SetConstantCamPositions(image_id, { 0 });
		}
		else
		{
			auto begin = layers.back().begin();
			const image_t image_id1 = *begin;
			begin++;
			image_t image_id2;
			if (begin == layers.back().end())
			{
				image_id2 = *(layers[layers.size() - 2].cbegin());
			}
			else
			{
				image_id2 = *begin;
		...
		for (const point3D_t point3D_id : point3D_ids) {
			...
			const size_t kMaxTrackLength = 30;
		...
		reconstruction_->numLocalBAImages.push_back(ba_config.NumImages());
		//std::cout << "num local BA images:" << ba_config.NumImages() << std::endl;
		BundleAdjuster bundle_adjuster(ba_options, ba_config);
		bundle_adjuster.Solve(reconstruction_.get(), finalCost);
	...
	for (const auto& pair : weights)
	{
		filter_image_ids.insert(pair.first);
	}
...
bool IncrementalMapper::AdjustGlobalBundle(
	const Options& options, const BundleAdjustmentOptions& ba_options, double& finalCost) {
	CHECK_NOTNULL(reconstruction_);
	...
	BundleAdjuster bundle_adjuster(ba_options, ba_config);
	if (!bundle_adjuster.Solve(reconstruction_.get(), finalCost)) {
		return false;
	}
...
std::shared_ptr<Reconstruction> IncrementalMapper::GetReconstruction()
{
	CHECK_NOTNULL(reconstruction_);
	return reconstruction_;
}
