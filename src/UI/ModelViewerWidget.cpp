...
const Eigen::Vector4f kNewRegistedImagePlaneColor(0.1f, 1.0f, 0.0f, 0.6f);
const Eigen::Vector4f kNewRegistedImageFrameColor(0.8f, 0.1f, 0.0f, 1.0f);
...
ModelViewerWidget::ModelViewerWidget(QWidget* parent) : QOpenGLWidget(parent), mouse_is_pressed_(false)
, focus_distance_(kInitFocusDistance), selected_image_id_(kInvalidImageId)
, selected_point3D_id_(kInvalidPoint3DId), coordinate_grid_enabled_(true)
, near_plane_(kInitNearPlane), lastRegistedImageID(-1)
, cPressed(false)
{
	...
	status_label = "0 Images - 0 Points";
}

void ModelViewerWidget::SetDatabase(Database* database)
{
	this->database = database;
}

void ModelViewerWidget::UpdateStatus(const QString& status)
{
	if (!status.isEmpty()) {
		status_label = status;
	}
...
void ModelViewerWidget::paintGL() {
	...
	quality_triangle_painter_.Render(pmv_matrix);
	guide_viewpoint_painter_.Render(pmv_matrix, 10.0f);
	guide_path_painter_.Render(pmv_matrix, width(), height(), 2.5f);
	guide_direction_painter_.Render(pmv_matrix, width(), height(), 2.5f);
}
...
void ModelViewerWidget::ReloadReconstruction() {
	...
	try
	{
		...
		QString labelString = QString::fromStdString(
			formatDuration(reconstruction->reconstructTime / 1000)
		);
		labelString += QString().asprintf("  %d Images - %d Points",
			static_cast<int>(reg_image_ids.size()),
			static_cast<int>(points3D.size())
		);

		// 时间统计
		double avgSeconds = reg_image_ids.empty() ? 0.0 :
			static_cast<double>(totalTimer.ElapsedMicroSeconds()) / reg_image_ids.size() / 1e6;
		labelString += QString().asprintf("  (Total time: %.2f s / image)", avgSeconds);
		UpdateStatus(labelString); // 在Upload渲染数据之前统计平均处理时间，以免造成多余时间统计 
...
void ModelViewerWidget::ReloadFeedback(bool showMesh, bool showQuality, bool showGuidePath)
{
	try
	{
		UploadMeshData(showMesh, !showQuality);
		UploadQualityData(showQuality);
		UploadPlanningData(showGuidePath);
		update();
	}
	catch (const std::exception& e)
	{
...
void ModelViewerWidget::StartTimer()
{
	totalTimer.Start();
}
...
void ModelViewerWidget::SetupPainters() {
	...
	quality_triangle_painter_.Setup();//质量评估三角面片
	guide_viewpoint_painter_.Setup();
	guide_direction_painter_.Setup();
	guide_path_painter_.Setup();
...
void ModelViewerWidget::UploadImageData(const bool selection_mode) {
	makeCurrent();
	image_t lastRegistedImageIDValue = lastRegistedImageID.load();
	std::vector<LinePainter::Data> line_data;
	...
	for (const image_t image_id : reg_image_ids) {
		...
		if (selection_mode) {
			...
			else if (image_id == lastRegistedImageIDValue)
			{
				plane_color = kNewRegistedImagePlaneColor;
				frame_color = kNewRegistedImageFrameColor;
			}
			else {
...
void ModelViewerWidget::UploadMeshData(bool showMesh, bool showMeshface, bool showMeshline)
{
	makeCurrent();
	...
	if (showMesh) {
		if (showMeshline) {
			line_data.reserve(render_data->mesh_lines.size());
			for (const auto& line : render_data->mesh_lines) {
				line_data.emplace_back(
					PointPainter::Data(line[0], line[1], line[2], line[3], line[4], line[5], line[6]),
					PointPainter::Data(line[7], line[8], line[9], line[10], line[11], line[12], line[13])
				);
			}
		}
		if (showMeshface) {
			triangle_data.reserve(render_data->mesh_data.size());
			for (const auto& pts : render_data->mesh_data) {
...
void ModelViewerWidget::UploadQualityData(bool showQuality)
{
	makeCurrent();

	std::vector<TrianglePainter::Data> triangle_data;
	if (showQuality) {
		triangle_data.reserve(render_data->quality_data.size());
		for (const auto& pts : render_data->quality_data) {
			triangle_data.emplace_back(
				PointPainter::Data(
					pts[0], pts[1], pts[2], pts[3], pts[4], pts[5], pts[6]
				),
	...
	quality_triangle_painter_.Upload(triangle_data);
}

void ModelViewerWidget::UploadPlanningData(bool showPath)
{
	makeCurrent();

	std::vector<PointPainter::Data> potint_data;
	std::vector<LinePainter::Data> guideline_data;
	std::vector<LinePainter::Data> direction_data;

	if (showPath) {
		potint_data.reserve(render_data->guide_points.size());
		for (const auto& pts : render_data->guide_points) {
			potint_data.emplace_back(
				PointPainter::Data(
					pts[0], pts[1], pts[2], pts[3], pts[4], pts[5], pts[6]
				)
			);
		}
		guideline_data.reserve(render_data->guide_lines.size());
		for (const auto& pts : render_data->guide_lines) {
			guideline_data.emplace_back(
				PointPainter::Data(
					pts[0], pts[1], pts[2], pts[3], pts[4], pts[5], pts[6]
				),
				PointPainter::Data(
		...
		direction_data.reserve(render_data->guide_directions.size());
		for (const auto& pts : render_data->guide_directions) {
			direction_data.emplace_back(
				PointPainter::Data(
					pts[0], pts[1], pts[2], pts[3], pts[4], pts[5], pts[6]
				),
				PointPainter::Data(
	...
	guide_viewpoint_painter_.Upload(potint_data);
	guide_path_painter_.Upload(guideline_data);
	guide_direction_painter_.Upload(direction_data);
}
