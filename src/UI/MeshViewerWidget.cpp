...
namespace
{
    ...
    std::string formatDuration(size_t seconds)
    {
        std::string result = "";
        ...
        if (minutes > 0 || hours > 0) { // Ensure minutes are added if there are hours
...
MeshViewerWidget::MeshViewerWidget(QWidget* parent) : QOpenGLWidget(parent) , mouse_is_pressed_(false) , focus_distance_(kInitFocusDistance) , selected_image_id_(kInvalidImageId) , selected_point3D_id_(kInvalidPoint3DId) , coordinate_grid_enabled_(true) , near_plane_(kInitNearPlane)
{
    ...
    status_label = "0 Images - 0 Points - 0 Faces";
}

void MeshViewerWidget::SetDatabase(Database* database)
{
    this->database = database;
}

void MeshViewerWidget::UpdateStatus(const QString& status)
{
    status_label = status;
    emit statusUpdate(status_label);
}
...
void MeshViewerWidget::paintGL() {
    ...
    // Mesh
    mesh_triangle_painter_.Render(pmv_matrix);
    mesh_line_painter_.Render(pmv_matrix , width() , height() , 1);
    //Guide
    guide_line_painter_.Render(pmv_matrix , width() , height() , 2.5f); // 渲染引导线
    marker_point_painter_.Render(pmv_matrix , 10.0f);                 // 渲染交点
    base_plane_painter_.Render(pmv_matrix);  // 绘制淡黄色基准平面
    heatmap_point_painter_.Render(pmv_matrix , 5.0f);
...
// 渲染点云模型
void MeshViewerWidget::ReloadReconstruction() {
    ...
    try
    {
        ...
        QString labelString = QString::fromStdString(formatDuration(0 / 1000));
        labelString += QString().asprintf("  %d Images - %d Points" , static_cast<int>( reg_image_ids.size() ) , static_cast<int>( points3D.size() ));
        UpdateStatus(labelString);
...
// 渲染实时反馈的Mesh模型
void MeshViewerWidget::ReloadReconstructionWithFeedback(bool showMeshline , bool showHeatmap , bool showGuidenceline) {
    if (reconstruction == nullptr) {
        return;
    }
    try
    {
        ...
        QString labelString = QString().asprintf("%d Images - %d Points" , static_cast<int>( reg_image_ids.size() ) , static_cast<int>( points3D.size() ));
        labelString += QString().asprintf(" - %d Faces" , static_cast<int>( mesh->faces.size() ));
        UpdateStatus(labelString);

        // UI界面更新
        Upload(); //渲染稀疏重建模型
        UploadMeshData(showMeshline); //渲染mesh表面
        UploadGuidenceData(showHeatmap , showGuidenceline); //渲染反馈数据
    }
    catch (const std::exception& e)
    {
        std::cout << "Unknown exception caught" << endl;
...
void MeshViewerWidget::ClearReconstruction() {
    cameras.clear();
    images.clear();
    points3D.clear();
    reg_image_ids.clear();
...
int MeshViewerWidget::GetProjectionType() const
{
    return 0;
}

void MeshViewerWidget::SetPointColormap(PointColormapBase* colormap) {
    point_colormap_.reset(colormap);
}

void MeshViewerWidget::SetImageColormap(ImageColormapBase* colormap) {
    image_colormap_.reset(colormap);
}

void MeshViewerWidget::UpdateMovieGrabber() {
    UploadMovieGrabberData();
    update();
}

void MeshViewerWidget::EnableCoordinateGrid() {
    coordinate_grid_enabled_ = true;
    update();
}

void MeshViewerWidget::DisableCoordinateGrid() {
    coordinate_grid_enabled_ = false;
    update();
}

void MeshViewerWidget::ChangeFocusDistance(const float delta) {
    if (delta == 0.0f) {
        return;
    }
...
void MeshViewerWidget::ChangeNearPlane(const float delta) {
    if (delta == 0.0f) {
        return;
    }
...
void MeshViewerWidget::ChangePointSize(const float delta) {
    if (delta == 0.0f) {
        return;
    }
...
void MeshViewerWidget::RotateView(const float x ,
    const float y ,
    const float prev_x ,
    const float prev_y) {
...
void MeshViewerWidget::TranslateView(const float x ,
    const float y ,
    const float prev_x ,
    const float prev_y) {
    if (x - prev_x == 0 && y - prev_y == 0) {
...
void MeshViewerWidget::ChangeCameraSize(const float delta) {
    if (delta == 0.0f) {
        return;
    }
...
QMatrix4x4 MeshViewerWidget::ModelViewMatrix() const {
    return model_view_matrix_;
}

void MeshViewerWidget::SetModelViewMatrix(const QMatrix4x4& matrix) {
    model_view_matrix_ = matrix;
    update();
}

void MeshViewerWidget::SelectObject(const int x , const int y) {
    makeCurrent();
...
void MeshViewerWidget::SelectMoviewGrabberView(const size_t view_idx) {
    selected_movie_grabber_view_ = view_idx;
    UploadMovieGrabberData();
    update();
}

QImage MeshViewerWidget::GrabImage() {
    makeCurrent();
...
void MeshViewerWidget::GrabMovie()
{
    //movie_grabber_widget_->show();
}

void MeshViewerWidget::ShowPointInfo(const point3D_t point3D_id)
{
    //point_viewer_widget_->Show(point3D_id);
}

void MeshViewerWidget::ShowImageInfo(const image_t image_id) {
    /*if (database)
    {
        CImageViewer* imageViewer = new CImageViewer(nullptr , database , image_id);
    }*/
}

float MeshViewerWidget::PointSize() const { return point_size_; }

float MeshViewerWidget::ImageSize() const { return image_size_; }

void MeshViewerWidget::SetPointSize(const float point_size) {
    point_size_ = point_size;
}

void MeshViewerWidget::SetImageSize(const float image_size) {
    image_size_ = image_size;
    UploadImageData();
}

void MeshViewerWidget::SetBackgroundColor(const float r ,
    const float g ,
    const float b) {
...
void MeshViewerWidget::SetupPainters() {
    makeCurrent();
    ...
    mesh_triangle_painter_.Setup();//Mesh表面
    mesh_line_painter_.Setup();//Mesh轮廓线
    marker_point_painter_.Setup();//紫色引导点(交点)
    guide_line_painter_.Setup();//引导线
    base_plane_painter_.Setup();//基面
    heatmap_point_painter_.Setup();//热力图点
...
void MeshViewerWidget::SetupView() {
...
void MeshViewerWidget::UploadCoordinateGridData() {
    makeCurrent();
...
void MeshViewerWidget::UploadPointData(const bool selection_mode) {
    makeCurrent();
...
void MeshViewerWidget::UploadPointConnectionData() {
    makeCurrent();
...
void MeshViewerWidget::UploadImageData(const bool selection_mode) {
    makeCurrent();
    ...
    for (const image_t image_id : reg_image_ids) {
        ...
        if (selection_mode) {
            ...
            /*else if (image_id == Reconstructor::lastRegImageID)
            {
                plane_color = Eigen::Vector4f(0.1f , 1.0f , 0.0f , 0.6f);
                frame_color = Eigen::Vector4f(0.8f , 0.1f , 0.0f , 1.0f);
            }*/
            else {
...
void MeshViewerWidget::UploadImageConnectionData() {
    makeCurrent();
...
void MeshViewerWidget::UploadMovieGrabberData() {
}

//上传Mesh模型数据
void MeshViewerWidget::UploadMeshData(bool showMeshline) {
    makeCurrent();
    ...
    if (!mesh) {
        mesh_line_painter_.Upload(line_data);
        mesh_triangle_painter_.Upload(triangle_data);
        update();
        return;
    }

    triangle_data.reserve(mesh->faces.size());
    if (showMeshline) {
        for (size_t idx = 0; idx < mesh->faces.size(); idx++) {
            PlyMeshVertex vertex1 = mesh->vertices[mesh->faces[idx].vertex_idx1];
            PlyMeshVertex vertex2 = mesh->vertices[mesh->faces[idx].vertex_idx2];
            PlyMeshVertex vertex3 = mesh->vertices[mesh->faces[idx].vertex_idx3];

            std::vector<std::pair<size_t , size_t>> edges = {
                {mesh->faces[idx].vertex_idx1, mesh->faces[idx].vertex_idx2},
                {mesh->faces[idx].vertex_idx2, mesh->faces[idx].vertex_idx3},
                {mesh->faces[idx].vertex_idx1, mesh->faces[idx].vertex_idx3}
            };
            for (auto& edge : edges) {
                ...
                if (drawnEdges.find(edge) == drawnEdges.end()) {
                    ...
                    line_data.emplace_back(
                        PointPainter::Data(mesh->vertices[edge.first].x , mesh->vertices[edge.first].y , mesh->vertices[edge.first].z , 0.0 , 0.7 , 0.0 , 1.0) ,
                        PointPainter::Data(mesh->vertices[edge.second].x , mesh->vertices[edge.second].y , mesh->vertices[edge.second].z , 0.0 , 0.7 , 0.0 , 1.0));
                }
            }
        }
    }
    else {
        for (const auto& pts : rendering_data->mesh_data) {
            triangle_data.emplace_back(
                PointPainter::Data(
                    pts[0] , pts[1] , pts[2] , pts[3] , pts[4] , pts[5] , pts[6]
                ) ,
                PointPainter::Data(
                    pts[7] , pts[8] , pts[9] , pts[10] , pts[11] , pts[12] , pts[13]
                ) ,
                PointPainter::Data(
                    pts[14] , pts[15] , pts[16] , pts[17] , pts[18] , pts[19] , pts[20]
                )
            );
        }
    }
...
//上传重建预测数据
void MeshViewerWidget::UploadGuidenceData(bool showHeatmap , bool showGuideline)
{
    makeCurrent();
    std::vector<PointPainter::Data> heatmap_points;
    std::vector<LinePainter::Data> guide_lines;
    std::vector<PointPainter::Data> guide_points;
    std::vector<TrianglePainter::Data> base_faces;

    if (!mesh) {
        guide_line_painter_.Upload(guide_lines);
        marker_point_painter_.Upload(guide_points);
        heatmap_point_painter_.Upload(heatmap_points);
        base_plane_painter_.Upload(base_faces);
        update();
        return;
    }

    if (showHeatmap) {
        for (const auto& pts : rendering_data->heatmap_points) {
            heatmap_points.emplace_back(PointPainter::Data(
                pts[0] , pts[1] , pts[2] , pts[3] , pts[4] , pts[5] , pts[6]
            ));
        }
    }
    if (showGuideline) {
        for (const auto& pts : rendering_data->guide_lines) {
        ...
        for (const auto& pts : rendering_data->guide_points) {
            guide_points.emplace_back(PointPainter::Data(
                pts[0] , pts[1] , pts[2] , pts[3] , pts[4] , pts[5] , pts[6]
            ));
        }
        for (const auto& pts : rendering_data->base_faces) {
            base_faces.emplace_back(
                PointPainter::Data(
                    pts[0] , pts[1] , pts[2] , pts[3] , pts[4] , pts[5] , pts[6]
                ) ,
                PointPainter::Data(
...
void MeshViewerWidget::ComposeProjectionMatrix() {
    projection_matrix_.setToIdentity();
    projection_matrix_.perspective(kFieldOfView , AspectRatio() , near_plane_ , kFarPlane);
}

float MeshViewerWidget::ZoomScale() const {
...
float MeshViewerWidget::AspectRatio() const {
    return static_cast<float>( width() ) / static_cast<float>( height() );
}

float MeshViewerWidget::OrthographicWindowExtent() const {
    return std::tan(DegToRad(kFieldOfView) / 2.0f) * focus_distance_;
}

Eigen::Vector3f MeshViewerWidget::PositionToArcballVector(
