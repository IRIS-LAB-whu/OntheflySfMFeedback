...
const size_t ColmapDatabase::kMaxNumImages =
static_cast<size_t>(std::numeric_limits<int32_t>::max());

const std::string ColmapDatabase::kInMemoryDatabasePath = ":memory:";

std::mutex ColmapDatabase::update_schema_mutex_;

ColmapDatabase::ColmapDatabase() : database_(nullptr) {}

ColmapDatabase::ColmapDatabase(const std::string& path) : ColmapDatabase() { Open(path); }

ColmapDatabase::~ColmapDatabase() { Close(); }

void ColmapDatabase::Open(const std::string& path) {
    Close();
...
bool ColmapDatabase::ExistsCamera(const camera_t camera_id) const {
    return ExistsRowId(sql_stmt_exists_camera_, camera_id);
}

bool ColmapDatabase::ExistsImage(const image_t image_id) const {
    return ExistsRowId(sql_stmt_exists_image_id_, image_id);
}

bool ColmapDatabase::ExistsImageWithName(const std::string& name) const {
    return ExistsRowString(sql_stmt_exists_image_name_, name);
}

bool ColmapDatabase::ExistsKeypoints(const image_t image_id) const {
    return ExistsRowId(sql_stmt_exists_keypoints_, image_id);
}

bool ColmapDatabase::ExistsDescriptors(const image_t image_id) const {
    return ExistsRowId(sql_stmt_exists_descriptors_, image_id);
}

bool ColmapDatabase::ExistsMatches(const image_t image_id1,
    const image_t image_id2) const {
    return ExistsRowId(sql_stmt_exists_matches_,
        ImagePairToPairId(image_id1, image_id2));
}

bool ColmapDatabase::ExistsInlierMatches(const image_t image_id1,
    const image_t image_id2) const {
    return ExistsRowId(sql_stmt_exists_two_view_geometry_,
        ImagePairToPairId(image_id1, image_id2));
}

size_t ColmapDatabase::NumCameras() const { return CountRows("cameras"); }

size_t ColmapDatabase::NumImages() const { return CountRows("images"); }

size_t ColmapDatabase::NumKeypoints() const { return SumColumn("rows", "keypoints"); }

size_t ColmapDatabase::MaxNumKeypoints() const {
    return MaxColumn("rows", "keypoints");
}

size_t ColmapDatabase::NumKeypointsForImage(const image_t image_id) const {
    return CountRowsForEntry(sql_stmt_num_keypoints_, image_id);
}

size_t ColmapDatabase::NumDescriptors() const {
    return SumColumn("rows", "descriptors");
}

size_t ColmapDatabase::MaxNumDescriptors() const {
    return MaxColumn("rows", "descriptors");
}

size_t ColmapDatabase::NumDescriptorsForImage(const image_t image_id) const {
    return CountRowsForEntry(sql_stmt_num_descriptors_, image_id);
}

size_t ColmapDatabase::NumMatches() const { return SumColumn("rows", "matches"); }

size_t ColmapDatabase::NumInlierMatches() const {
    return SumColumn("rows", "two_view_geometries");
}

size_t ColmapDatabase::NumMatchedImagePairs() const { return CountRows("matches"); }

size_t ColmapDatabase::NumVerifiedImagePairs() const {
    return CountRows("two_view_geometries");
}

Camera ColmapDatabase::ReadCamera(const camera_t camera_id) const {
...
std::vector<Camera> ColmapDatabase::ReadAllCameras() const {
...
Image ColmapDatabase::ReadImage(const image_t image_id) const {
...
Image ColmapDatabase::ReadImageWithName(const std::string& name) const {
...
std::vector<Image> ColmapDatabase::ReadAllImages() const {
...
FeatureKeypoints ColmapDatabase::ReadKeypoints(const image_t image_id) const {
...
FeatureDescriptors ColmapDatabase::ReadDescriptors(const image_t image_id) const {
...
FeatureMatches ColmapDatabase::ReadMatches(image_t image_id1,
...
std::vector<std::pair<image_pair_t, FeatureMatches>> ColmapDatabase::ReadAllMatches()
const {
...
TwoViewGeometry ColmapDatabase::ReadTwoViewGeometry(const image_t image_id1,
    const image_t image_id2) const {
    const image_pair_t pair_id = ImagePairToPairId(image_id1, image_id2);
    SQLITE3_CALL(
...
void ColmapDatabase::ReadTwoViewGeometries(
...
void ColmapDatabase::ReadTwoViewGeometryNumInliers(
...
camera_t ColmapDatabase::WriteCamera(const Camera& camera,
...
image_t ColmapDatabase::WriteImage(const Image& image,
...
void ColmapDatabase::WriteKeypoints(const image_t image_id,
...
void ColmapDatabase::WriteDescriptors(const image_t image_id,
...
void ColmapDatabase::WriteMatches(const image_t image_id1,
...
void ColmapDatabase::WriteTwoViewGeometry(
...
void ColmapDatabase::UpdateCamera(const Camera& camera) const {
    SQLITE3_CALL(
...
void ColmapDatabase::UpdateImage(const Image& image) const {
...
void ColmapDatabase::DeleteMatches(const image_t image_id1,
    const image_t image_id2) const {
    const image_pair_t pair_id = ImagePairToPairId(image_id1, image_id2);
    SQLITE3_CALL(sqlite3_bind_int64(
...
void ColmapDatabase::DeleteInlierMatches(const image_t image_id1,
    const image_t image_id2) const {
    const image_pair_t pair_id = ImagePairToPairId(image_id1, image_id2);
...
void ColmapDatabase::ClearAllTables() const {
    ClearMatches();
...
void ColmapDatabase::Merge(const ColmapDatabase& database1,
    const ColmapDatabase& database2,
    ColmapDatabase* merged_database) {
    // Merge the cameras.
    ...
    for (const auto& matches : database1.ReadAllMatches()) {
        ...
        ColmapDatabase::PairIdToImagePair(matches.first, &image_id1, &image_id2);
...

    {
        ...
        for (size_t i = 0; i < two_view_geometries.size(); ++i) {
            image_t image_id1, image_id2;
            ColmapDatabase::PairIdToImagePair(image_pair_ids[i], &image_id1, &image_id2);
...
void ColmapDatabase::BeginTransaction() const {
    SQLITE3_EXEC(database_, "BEGIN TRANSACTION", nullptr);
}

void ColmapDatabase::EndTransaction() const {
    SQLITE3_EXEC(database_, "END TRANSACTION", nullptr);
}

void ColmapDatabase::PrepareSQLStatements() {
    sql_stmts_.clear();
...
void ColmapDatabase::FinalizeSQLStatements() {
    for (const auto& sql_stmt : sql_stmts_) {
        SQLITE3_CALL(sqlite3_finalize(sql_stmt));
    }
}

void ColmapDatabase::CreateTables() const {
    CreateCameraTable();
    CreateImageTable();
...
void ColmapDatabase::CreateCameraTable() const {
...
void ColmapDatabase::CreateImageTable() const {
...
void ColmapDatabase::CreateKeypointsTable() const {
    const std::string sql =
...
void ColmapDatabase::CreateDescriptorsTable() const {
    const std::string sql =
...
void ColmapDatabase::CreateMatchesTable() const {
    const std::string sql =
...
void ColmapDatabase::CreateTwoViewGeometriesTable() const {
...
void ColmapDatabase::UpdateSchema() const {
    ...
    const std::string update_user_version_sql =
        StringPrintf("PRAGMA user_version = %d;", 3900);
    SQLITE3_EXEC(database_, update_user_version_sql.c_str(), nullptr);
}

bool ColmapDatabase::ExistsTable(const std::string& table_name) const {
    const std::string sql =
...
bool ColmapDatabase::ExistsColumn(const std::string& table_name,
...
bool ColmapDatabase::ExistsRowId(sqlite3_stmt* sql_stmt,
...
bool ColmapDatabase::ExistsRowString(sqlite3_stmt* sql_stmt,
...
size_t ColmapDatabase::CountRows(const std::string& table) const {
    const std::string sql =
...
size_t ColmapDatabase::CountRowsForEntry(sqlite3_stmt* sql_stmt,
    const sqlite3_int64 row_id) const {
...
size_t ColmapDatabase::SumColumn(const std::string& column,
...
size_t ColmapDatabase::MaxColumn(const std::string& column,
    const std::string& table) const {
    const std::string sql =
...
DatabaseTransaction::DatabaseTransaction(ColmapDatabase* database)
...
DatabaseExporter::DatabaseExporter(Database* sourceDatabase, const std::string& targetPath)
{
    CHECK(sourceDatabase);
    this->sourceDatabase = sourceDatabase;

    if (ExistsFile(targetPath))
    {
        const std::filesystem::path filePath = targetPath;
        std::filesystem::remove(filePath);
    }

    targetDatabase = new ColmapDatabase(targetPath);
}
DatabaseExporter::~DatabaseExporter()
{
    delete targetDatabase;
}
void DatabaseExporter::Export()
{
    CHECK(sourceDatabase && targetDatabase);

    const vector<Camera> cameras = sourceDatabase->ReadAllCameras();
    const vector<Image> images = sourceDatabase->ReadAllImages();
    const vector<pair<image_pair_t, FeatureMatches>> matches = sourceDatabase->ReadAllMatches();
    const image_t numImages = images.size();
    
    DatabaseTransaction database_transaction(targetDatabase);
    for (const Camera& camera : cameras)
    {
        targetDatabase->WriteCamera(camera, true);
    }
    for (const Image& image : images)
    {
        targetDatabase->WriteImage(image, true);
    }
    for (image_t imageID = 1; imageID <= numImages; imageID++)
    {
        targetDatabase->WriteKeypoints(imageID, sourceDatabase->ReadKeypoints(imageID));
        targetDatabase->WriteDescriptors(imageID, sourceDatabase->ReadDescriptors(imageID));
    }
    for (const pair<image_pair_t, FeatureMatches>& pair : matches)
    {
        image_t imageID1, imageID2;
        ColmapDatabase::PairIdToImagePair(pair.first, &imageID1, &imageID2);
        targetDatabase->WriteMatches(imageID1, imageID2, pair.second);
        if (sourceDatabase->ExistsInlierMatches(imageID1, imageID2))
        {
            targetDatabase->WriteTwoViewGeometry(imageID1, imageID2, sourceDatabase->ReadTwoViewGeometry(imageID1, imageID2));
        }
    }
}
