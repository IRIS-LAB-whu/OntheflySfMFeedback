...
public:
    ...
    ColmapDatabase();
    explicit ColmapDatabase(const std::string& path);
    ~ColmapDatabase();
    ...
    static void Merge(const ColmapDatabase& database1,
...
public:
    explicit DatabaseTransaction(ColmapDatabase* database);
    ~DatabaseTransaction();

private:
    NON_COPYABLE(DatabaseTransaction)
        NON_MOVABLE(DatabaseTransaction)
        ColmapDatabase* database_;
...
image_pair_t ColmapDatabase::ImagePairToPairId(const image_t image_id1,
...
void ColmapDatabase::PairIdToImagePair(const image_pair_t pair_id,
    image_t* image_id1,
    image_t* image_id2) {
...
// image order to generate unique image pair identifiers independent of the
// order in which the image identifiers are used.
bool ColmapDatabase::SwapImagePair(const image_t image_id1, const image_t image_id2) {
...
{
public:
    explicit DatabaseExporter(Database* sourceDatabase, const std::string& targetPath);
    ~DatabaseExporter();
    void Export();

private:
    Database* sourceDatabase = nullptr;
    ColmapDatabase* targetDatabase = nullptr;
};
